...
import bait2073.mad.assignments.databinding.FragmentPilatesExercisesBinding

class PilatesExercisesFragment : Fragment() {

    private var _binding: FragmentPilatesExercisesBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPilatesExercisesBinding.inflate(inflater, container, false)
        ...
        val videoUri = Uri.parse("android.resource://" + requireContext().packageName + "/" + R.raw.pilates)

        // Set the video URI to the VideoView
        binding.PilatesVideo.setVideoURI(videoUri)

        // Set OnClickListener to start/stop video playback when the user clicks on the VideoView
        binding.PilatesVideo.setOnClickListener {
            if (!isVideoStarted) {
                // Start video playback if it hasn't been started
                binding.PilatesVideo.start()
                isVideoStarted = true
            } else {
                // Pause video if it's already playing
                if (binding.PilatesVideo.isPlaying) {
                    binding.PilatesVideo.pause()
                } else {
        ...
        completeButton.setOnClickListener {
            ...
            if (userId != null) {
                completeExercise(userId, "PilatesExercise")
            } else {
