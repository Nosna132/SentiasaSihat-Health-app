...
import bait2073.mad.assignments.databinding.FragmentBodyWeightFragmentBinding

class body_weight_fragment : Fragment() {

    private var _binding: FragmentBodyWeightFragmentBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentBodyWeightFragmentBinding.inflate(inflater, container, false)
        ...
        val videoUri = Uri.parse("android.resource://" + requireContext().packageName + "/" + R.raw.bodyweight)

        // Set the video URI to the VideoView
        binding.BodyWeightVideo.setVideoURI(videoUri)

        // Set OnClickListener to start/stop video playback when the user clicks on the VideoView
        binding.BodyWeightVideo.setOnClickListener {
            if (!isVideoStarted) {
                // Start video playback if it hasn't been started
                binding.BodyWeightVideo.start()
                isVideoStarted = true
            } else {
                // Pause video if it's already playing
                if (binding.BodyWeightVideo.isPlaying) {
                    binding.BodyWeightVideo.pause()
                } else {
        ...
        completeButton.setOnClickListener {
            ...
            if (userId != null) {
                completeExercise(userId, "BodyWeightFitness")
            } else {
